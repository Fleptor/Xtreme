...
export async function build() {
  ...
  const ambassadorMarkup = ambassadors.map(([university,name,id,source,originalUniversity],i) => `<article class="directory-card" data-search="${escape(originalUniversity)}"><h4>${escape(university)}</h4><p>${escape(name)}</p><div class="copy-field"><input id="directory-id-${i}" value="${escape(id)}" readonly dir="ltr" spellcheck="false" aria-label="رقم سفير ${escape(university)}"><button data-copy-target="directory-id-${i}" aria-label="نسخ رقم سفير ${escape(university)}">نسخ</button></div><a class="text-link" href="sources.html#ambassador-sources">مصدر قائمة السفراء ↗</a></article>`).join('');
  const ambassadorOptions = ambassadors.map(([university,name,id,source,originalUniversity]) => {
    const aliases = originalUniversity.startsWith('University of Jordan') ? 'UJ JU' : originalUniversity.startsWith('Al-Balqa') ? 'BAU' : '';
    return `<option value="${escape(id)}" data-name="${escape(name)}" data-university="${escape(university)}" data-search="${escape(originalUniversity + ' ' + aliases)}"${id === 'IEEEXTREME20SB08217' ? ' selected' : ''}>${escape(university)} — ${escape(name)}</option>`;
  }).join('');
  const html = template.replace('{{AMBASSADOR_OPTIONS}}', ambassadorOptions).replace('{{AMBASSADORS}}', ambassadorMarkup).replace('{{FAQ}}', faqMarkup).replace('{{CATEGORIES}}',categoryMarkup).replace('{{RESOURCES}}',resourceMarkup).replace('{{SOCIALS}}',socialMarkup).replaceAll('{{FAQ_COUNT}}',String(faqs.length)).replace(/\{\{link:(\w+)\}\}/g,(_,key) => { if(!links[key]) throw new Error(`Unknown link ${key}`); return escape(links[key]); });
